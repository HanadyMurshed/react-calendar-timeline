...
exports.getCanvasWidthFactor = getCanvasWidthFactor;
...
function iterateTimes(start, end, unit, timeSteps, callback) {
  ...
  while (time.valueOf() < end) {
    var nextTime = (0, _moment["default"])(time).add(timeSteps[unit] || 1, "".concat(unit, "s")).startOf(unit);
...
function getCanvasWidthFactor() {
  var shouldExpandCanvas = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : true;
...
function getCanvasBoundariesFromVisibleTime(visibleTimeStart, visibleTimeEnd) {
  var shouldExpandCanvasBoundaries = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : true;
...
function calculateScrollCanvas(visibleTimeStart, visibleTimeEnd, forceUpdateDimensions, items, groups, props, state) {
  ...
  if (!canKeepCanvas || forceUpdateDimensions) {
    var _getCanvasBoundariesF = getCanvasBoundariesFromVisibleTime(visibleTimeStart, visibleTimeEnd, props.resizableCanvas),
    ...
    var canvasWidth = getCanvasWidth(mergedState.width, state.canvasWidthFactor); // The canvas cannot be kept, so calculate the new items position
